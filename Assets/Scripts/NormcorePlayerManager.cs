...
public class NormcorePlayerManager : MonoBehaviour {
    ...
    private void DidConnectToRoom(Realtime realtime) {
        ...
        GameObject playerGameObject = Realtime.Instantiate(prefabName: "Player and Camera Parent",  // Prefab name
                                      ownedByClient: true,      // Make sure the RealtimeView on this prefab is owned by this client
                                      preventOwnershipTakeover: true,      // Prevent other clients from calling RequestOwnership() on the root RealtimeView.
                                      useInstance: realtime); // Use the instance of Realtime that fired the didConnectToRoom event.

        // Get a reference to the player
        KeyboardPlayerController player = playerGameObject.GetComponent<KeyboardPlayerController>();
